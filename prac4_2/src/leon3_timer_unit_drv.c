...
struct Timer_regs_t{
	volatile uint32_t CounterValue; 	/** \brief Timer Counter Value Register */
	volatile uint32_t ReloadValue; 		/** \brief Timer Reload Value Register */
	volatile uint32_t Ctrl; 			/** \brief Timer Control Register */
...
{
	volatile uint32_t ScalerValue;			/* 0x80000300 */ /** \brief Timer Unit Scaler Register */
	volatile uint32_t ScalerReloadValue; 	/* 0x80000304 */ /** \brief Timer Unit Scaler Reload Value Register */
	volatile uint32_t ConfigReg; 			/* 0x80000308 */ /** \brief Timer Unit Configuration Register */
	uint32_t 		  reserved; 			/* 0x8000030A */
	volatile struct Timer_regs_t			Timer[LEON3_TIMER_UNIT_TIMERS]; /** \brief Registers of the Timers*/ //Valor que define cuantos timers hay
};
...
uint8_t leon3_timer_config(uint8_t timerId
	...
	if(timerId < LEON3_TIMER_UNIT_TIMERS){
		...
		//TODO
		//Configurar los valores "CounterValue" y "ReloadValue" del "timerId"
		...
		if (chain_with_prec_timer == false) {
