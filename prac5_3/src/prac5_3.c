...
//DECLARACION DE IDENTIFICADOR DE MUTEX (MutexId)
//Y EL NOMBRE COMO VARIABLE GLOBAL (MutexName)
rtems_id MutexId;
...
//TODO
rtems_task TAvoidObstacles(rtems_task_argument argument);
//TODO
rtems_task TPathTracking(rtems_task_argument argument);
//TODO
rtems_task TSensorFusion(rtems_task_argument argument);
//TODO
rtems_task TCalculatePath(rtems_task_argument argument);
...
{
	//CREACION DEL SEMAFORO DENTRO DE LA FUNCION INIT
  ...
  time.second = 0;
  time.ticks  = 0;
  rtems_clock_set(&time) ;//added


  Task_name[ 1 ] = rtems_build_name( 'T', 'A', '1', ' ' );
  Task_name[ 2 ] = rtems_build_name( 'T', 'A', '2', ' ' );
  Task_name[ 3 ] = rtems_build_name( 'T', 'A', '3', ' ' );
  Task_name[ 4 ] = rtems_build_name( 'T', 'A', '4', ' ' );//added
  ...
  //Pasar  los nombres de las funciones que usas como parametro en rtems_task_start
...
//Task_delay_until se usa para sustituir rtems_task_wake_after
//por task_delay_until... que es la función que uso en las 4 funciones de abajo
// a continuacion
...
//modificad las cuatro tareas anteriores para que sustituyan las llamadas a la
//función rtems_task_wake_after por llamadas a la función task_delay_until .
rtems_task TAvoidObstacles (rtems_task_argument unused) {
	...
	for(;;){
		rtems_semaphore_obtain(1,RTEMS_WAIT,0);//Added
		puts("T1 Do Avoid Obstacles");
		printf(" - rtems_ticks_since_boot - %i\n\n",
					get_ticks_since_boot());

		//rtems_task_wake_after(10);//modificado
		rtems_semaphore_release(1);
		ticks+=10;
		task_delay_until(ticks);//Added
	}
...
rtems_task TPathTracking (rtems_task_argument unused) {
	...
	rtems_interval ticks = get_ticks_since_boot();
	for(;;){
		rtems_semaphore_obtain(1,RTEMS_WAIT,0);
