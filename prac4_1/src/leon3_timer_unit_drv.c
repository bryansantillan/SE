...
uint8_t leon3_timer_config(uint8_t timerId
					, uint32_t timerValue
	...
	if(timerId < LEON3_TIMER_UNIT_TIMERS ){
		...
		/*
		 * PARA ACTIVAR:
		 * |= valor a escribir ---  poner a 1 los bits a 1 de la mascara y mantener el resto
		 *
		 * PARA DESACTIVAR:
		 * &= ~(valor a escribir) --- poner a 0 los bits a 1 de la mascara y mantener el resto
		 * ~  == not
		 * ~(00100) == (11011)
		 */


		//Configurar los valores (parámetros) "CounterValue" y "ReloadValue" del "timerId"
		//con time Value
		...
		//Configure the field Chain with precedent timer (without changing the rest of the fields of Ctrl) of the timerId
		//Use the LEON3 Timer Ctrl Masks
		if (chain_with_prec_timer){
			//cambia a 1 solo los bits que estan a 1 en la máscara a 1, mantiene el valor del resto
			pLEON3_TimerUnit_REGS->Timer[timerId].Ctrl |= (LEON3_TIMER_CHAIN_WITH_PREC_TIMER) ;
		}else{
			//cambia a 0 solo los bits que estan a 1 en la máscara a 1, mantiene el valor del resto
			pLEON3_TimerUnit_REGS->Timer[timerId].Ctrl &= ~ (LEON3_TIMER_CHAIN_WITH_PREC_TIMER) ;
		}
		//TODO
		...
		//Usa las máscaras definidas en LEON3 Timer Ctrl Masks
		//Configure the Restart field of timerId (without changing the rest of the fields of Ctrl) that determines if timer must be restarted after underflow
		//Use the LEON3 Timer Ctrl Masks
		if(restart_timer){
...
uint8_t leon3_timer_enable_irq(uint8_t timerId){
	...
	if(timerId < LEON3_TIMER_UNIT_TIMERS ){
		...
		//Usa las máscaras definidas en LEON3 Timer Ctrl Masks
		//Enable the timerId interrupts without changing the rest of the fields of Ctrl
		//Use the LEON3 Timer Ctrl Masks
...
uint8_t leon3_timer_disable_irq(uint8_t timerId){
	...
	if(timerId < LEON3_TIMER_UNIT_TIMERS ){
		...
		//Usa las máscaras definidas en LEON3 Timer Ctrl Masks
		//Disable the timerId interrupts without changing the rest of the fields of Ctrl
		//Use the LEON3 Timer Ctrl Masks
...
uint8_t leon3_timer_enable(uint8_t timerId){
	...
	if(timerId < LEON3_TIMER_UNIT_TIMERS ){
		...
		//Usa las máscaras definidas en LEON3 Timer Ctrl Masks
		//Enable the timerId without changing the rest of the fields of Ctrl
		//Use the LEON3 Timer Ctrl Masks
...
uint8_t leon3_timer_disable(uint8_t timerId){
	...
	if(timerId < LEON3_TIMER_UNIT_TIMERS ){
		...
		//Usa las máscaras definidas en LEON3 Timer Ctrl Masks
		//Disable the timerId without changing the rest of the fields of Ctrl
		//Use the LEON3 Timer Ctrl Masks
...
uint8_t leon3_timer_clear_irq(uint8_t timerId){
	...
	if(timerId < LEON3_TIMER_UNIT_TIMERS ){
		...
		//Usa las máscaras definidas en LEON3 Timer Ctrl Masks
		//Clear the field Interrupt Pending of the timerId without changing the rest of the fields of Ctrl
		//Use the LEON3 Timer Ctrl Masks

		//OJO QUE DICE QUE HAY QUE PONER A 0 el bit de Clear!, por eso el Registro &= ~(mask)
